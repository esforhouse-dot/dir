...
class CadCore {
    ...
    handleMapClick(e) {
        if (state.isCenterMode) return; // В режиме прицела клики по карте не работают
        ...
        this.processActionAtCoords(coords);
    }

    // Универсальный метод добавления по координатам (от мыши или от центра)
    processActionAtCoords(coords) {
        if (!coords) return;
    ...
    actionFromCenter() {
        ...
        this.processActionAtCoords(center);
    }
        ...
        obj.events.add('mouseenter', (e) => {
            ...
            if (type === 'LineString' || type === 'Polygon') {
                if (this.glowEffect) {
                    ...
                    this.glowEffect.options.set('strokeWidth', objWidth + 10);
                    this.glowEffect.options.set('strokeOpacity', 0.4);
    ...
    enableVisualAids(polyline) {
        ...
        this._moveHandler = (e) => {
            ...
            if(coords.length > 0) {
                ...
                // В мобильном режиме cursor берется от центра, если нужно,
                // но пока для drawing лучше оставить мышь/тач, т.к. рисовать
                // сложную линию центром экрана неудобно (только точки ставить удобно).
                // Поэтому тут оставляем как есть.
