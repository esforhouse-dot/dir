...
class UiManager {
    init() {
        ...
        // --- НОВОЕ: Режим прицела (Target Mode) ---
        const btnTarget = document.getElementById('btn-target-mode');
        const targetCrosshair = document.getElementById('center-target');
        const btnAction = document.getElementById('btn-action-main');

        btnTarget.onclick = () => {
            state.isCenterMode = !state.isCenterMode;
            btnTarget.classList.toggle('active', state.isCenterMode);
            
            if (state.isCenterMode) {
                targetCrosshair.classList.add('active');
                btnAction.classList.add('active');
                this.showNotification('Режим "Прицел" включен');
            } else {
                targetCrosshair.classList.remove('active');
                btnAction.classList.remove('active');
                this.showNotification('Режим "Клик" включен');
            }
        };

        // --- НОВОЕ: Кнопка действия (FAB) ---
        btnAction.onclick = (e) => {
            e.preventDefault(); 
            e.stopPropagation();
            
            if (!state.tool && !state.isDrawing) {
                this.showFloatingToolsMenu();
                this.showNotification('Выберите инструмент');
                return;
            }

            cadCore.actionFromCenter();
