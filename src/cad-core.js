...
const DEFAULT_NAMES = {
    pole: 'Опора',
    light: 'Светильник',
    cabinet: 'Щит электрический',
    flag: 'Флаг',
    star: 'Звезда',
    substation: 'ТП (подстанция)',
    subscriber: 'Абонент',
    line: 'Линия',
    polygon: 'Зона (Полигон)',
    text: 'Текст',
    task: 'Задача'
};
...
class CadCore {
    ...
    placePointObject(type, coords, text = '', groupId = null, iconColor = null) {
        ...
        switch(type) {
            case 'light': layoutClass = createDynamicLayout('icon-light'); break;
            ...
            case 'task': layoutClass = createDynamicLayout('icon-task'); break; 
            default: layoutClass = createDynamicLayout('icon-light');
        }
        ...
        if (type === 'task') {
            placemark.properties.set('taskData', { status: 'active', comment: '', text: text || 'Новая задача' });
            placemark.properties.set('iconColor', '#00AEEF');
        }
    ...
    attachObjectEvents(obj) {
        ...
        obj.events.add('click', (e) => {
            ...
            if (type === 'Point') {
                if (subType === 'text') {
                    ...
                    if(window.App.ui) window.App.ui.openTaskModal(obj);
                } else {
        ...
        obj.events.add('contextmenu', (e) => {
             ...
             if(window.App.ui) {
                 if (subType === 'task') {
                     window.App.ui.openTaskModal(obj);
                 } else if (type === 'Point' || type === 'Polygon') {
                     window.App.ui.openRenameModal(obj);
                 } else if (type === 'LineString') {
                     window.App.ui.openCableModal(obj);
                 }
             }
        });
    }
    ...
    enableVisualAids(polyline) {
        if (!this.ghostLine) {
             const safe = [[0,0], [0.0001, 0.0001]];
             this.ghostLine = new ymaps.Polyline(safe, {}, { strokeColor: '#fff', strokeWidth: 2, strokeStyle: 'dash', visible: false });
                ...
                try {
                    ...
                    if(coords && coords.length > 0) {
                        ...
                        if (last && cursor) {
                            ...
                            if (!this.ghostLine.options.get('visible')) {
    ...
    disableVisualAids() {
        if(this.ghostLine) {
            this.ghostLine.options.set('visible', false);
        }
        if(this._moveHandler) {
            this.map.events.remove('mousemove', this._moveHandler);
            this._moveHandler = null;
